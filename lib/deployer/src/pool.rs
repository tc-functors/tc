...
async fn add_permission(auth: &Auth, lambda_arn: &str, pool_id: &str) {
    ...
    let _ = lambda::add_permission(
        client, lambda_arn, principal, &source_arn, pool_id
    ).await;
}
...
pub async fn create(auth: &Auth, pools: HashMap<String, Pool>) {
    ...
    for (name, pool) in pools {
        ...
        let pool_id = cognito::create_or_update_pool(&client, &name, mappings.clone(), email_config).await;

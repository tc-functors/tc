...
pub async fn scaffold_function(name: &str, infra_dir: &str) {
    ...
    let env = provider::init(Some(String::from("{{env}}")), None, Config::new(None, "")).await;
    scaffold::write_role(name, &role_dir, &env.base_lambda_policy());
    scaffold::write_vars(name, &vars_dir);
    scaffold::write_function_spec(name);
}
